...
#define DEBUGPIN 4
float ADC=3.3;//change to 1.0 for esp8266 12e standalone
...
void log_data()
{
  //printing the all information telementry
    ...
    terminal.println("BATTERY: "+String(voltage)+" V");
...
void voltage_measure()
{
 int v= analogRead(A0);
 int batt=0;
 voltage = v * (ADC/1024)*((R1 + R2)/R2);
 if( voltage>=6.0)
 {
 batt=map(v,6.0,8.5,0,100);
...
void setup() {
  ...
  status_auth=configStore.getFlag(CONFIG_FLAG_VALID);
  if(Reset_Wifi)
  {
  enterResetConfig();
  }
